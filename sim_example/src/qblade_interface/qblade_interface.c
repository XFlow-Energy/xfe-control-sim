 ...
 * input parameters (`omega`, `tau_gen`, `time_sec`, `dt_sec`,
...
void example_qblade_interface(QBLADE_INTERFACE_PARAM_LIST)
{
	...
	*omega = avr_swap[REC_MEASURED_ROTOR_SPEED];
	...
	if (accumulated_Time >= *control_Dt_Sec)
	{
		...
		turbine_control(dynamic_data, fixed_data); // update the generator torque command
	...
	avr_swap[REC_DEMANDED_GENERATOR_TORQUE] = (float)(*tau_Gen);
