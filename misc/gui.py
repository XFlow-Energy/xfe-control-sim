...
class CSVEditorApp:
	...
	def edit_config_window(self):
		...
		editable_vars = [
			...
			"DRIVETRAIN_FILENAME",
			"NUMERICAL_INTEGRATOR_FILENAME",
			"EOM_FILENAME",
			"FLOW_GEN_FILENAME",
			"FLOW_BTS_FILENAME",
			"FLOW_CSV_FILENAME",
			"TURBINE_CONTROL_FILENAME",
			"AERO_MODEL_FILENAME",
			"QBLADE_INTERFACE_FILENAME",
			"DATA_PROCESSING_FILENAME",
			"BUILD_SHARED_LIBS",
			"BUILD_AERO_CONTROL_EXECUTABLE",
			"BUILD_XFE_SCADA_INTERFACE",
			"INTEGRATE_CUSTOMER_MODELS",
			"RUN_SINGLE_MODEL_ONLY",
			"CUSTOMER_NAME",
			"GIT_TAG_TO_USE"
		]
		...
		for var in editable_vars:
			...
			row += 1
		save_btn = tk.Button(self.config_window, text="Save Config", command=lambda: self.save_config(self.config_window))
